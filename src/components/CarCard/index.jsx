...
function CarCard({ postData }) {
  ...
  const { billionPart, millionPart } = VNDFormatToWord(
    postData.car.selling_price
  );
  ...
  const handleAddWishList = async (post_id) => {
    ...
    if (
      ...
      try {
        await customerService.addWishList({
          post_id: post_id,
  ...
  const handleRemoveWishList = async (post_id) => {
    ...
    if (
      ...
      dispatch(setLoading(true));
      try {
        await customerService.removeWishList({
          post_id: post_id,
          access_token: access_token,
        });
  ...
  return (
    <div className="rounded-2xl h-full flex flex-col shadow-md relative">
      <div className="h-72 relative">
        <CarImageSlide
          images={postData.car.car_galleries}
          srcThumb={postData.car.car_galleries[0].gallery_url}
        />
      </div>

      <div className="flex flex-col flex-auto justify-between bg-[white] rounded-b-2xl relative">
        <Link
          to={`/${postData.car.car_slug}`}
            ...
            {(postData.package_option === "VIP" ||
              ...
              <div className="flex space-x-2">
                <div className="flex space-x-2 items-center bg-primary-color text-grey-color w-fit py-1 px-2 rounded-2xl text-xs">
                  ...
                  <span className="font-bold">{postData.total_view}</span>
                </div>
                <div className="flex space-x-2 items-center bg-primary-color text-grey-color w-fit py-1 px-2 rounded-2xl text-xs">
                  <FontAwesomeIcon icon={faHeart} />
                  <span className="font-bold">{postData.total_like}</span>
                </div>
              </div>
            )}

            <p className="mt-3 text-xl font-bold hover:text-primary-color capitalize col-span-2">
          ...
          <div className="mb-2" style={{ fontSize: "12px" }}>
              ...
              <div className="w-1/2">
                <span className="flex space-x-2 items-center">
                  ...
                  <span className="truncate font-semibold text-sm">
                    {postData.car.manufacturing_date ?? "-"}
                  </span>
                </span>
              </div>
              ...
              <div className="truncate w-1/2 mt-2">
                <span className="flex space-x-2 items-center">
                  <img
                    height="15"
                    width="15"
                    src="/img/car-detail/car-detail-icon/steering-wheel-lg.png"
                    alt=""
                  />
                  <span className="truncate font-semibold text-sm">
                    {postData.car.transmission ?? "-"}
                  </span>
                </span>
              </div>
              <div className="truncate w-1/2 mt-2">
            ...
            <div className="mt-2 text-2xl text-right text-primary-color">
              <span className="font-bold">
                ...
                {billionPart ? (
                  <span className="px-1 text-xl font-semibold">billion</span>
                ) : (
                  ""
                )}
                {millionPart}
        ...
        <div className="flex items-center justify-end absolute z-50 top-4 right-4 cursor-pointer">
          {isFavorite ? (
            <BookmarkIcon
              onClick={() => handleRemoveWishList(postData.id)}
              className="text-[#FDCF33] !w-8 !h-8"
            />
          ) : (
            <BookmarkBorderIcon
              className="text-[#FDCF33] !w-8 !h-8"
              onClick={() => handleAddWishList(postData.id)}
            />
          )}
        </div>
      </div>
